...
void main()
{
    ...
    // Pattern 22
    // float strength =  max(abs(vUv.x - 0.5), abs(vUv.y - 0.5));
    // // strength = step(0.3, strength) - step(0.4, strength);
    // strength = step(0.2, strength) * (1.0 - step(0.25, strength));
    // gl_FragColor = vec4(vec3(strength), 1.0);  
    ...
    // CLAMP the streng 
    // Prevent value to go below 0 and upper 1
    // strength = clamp(streng, 0.0, 1.0);
